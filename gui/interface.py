...
class ChooseMode():
    ...
    def drawMenu(self):
        ...
        menu_text = menu_font.render('CHOOSE YOUR OPPONENT: ', True, 'white')
        menu_board.blit(menu_text, (50, 25))
        self.screen.blit(menu_board, menu_board_rect)

        pygame.display.update()

    # Check which oppenent the player has chosen
    def checkOpponentChoice(self, button_vsbot, button_vsplayer, pos):
        if button_vsbot.rect.collidepoint(pos):
            self.mode = 1
        elif button_vsplayer.rect.collidepoint(pos):
            self.mode = 2
